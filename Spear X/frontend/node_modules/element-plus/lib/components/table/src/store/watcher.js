...
function useWatcher() {
  ...
  const updateColumns = () => {
    ...
    if (types.isUndefined(selectionInitialFixed) && _columns.value[0] && _columns.value[0].type === "selection") {
  ...
  const toggleRowSelection = (row, selected, emitChange = true, ignoreSelectable = false) => {
    ...
    const changed = util.toggleRowStatus(selection.value, row, selected, treeProps, ignoreSelectable ? void 0 : selectable.value);
    if (changed) {
