...
func (a *App) GetNewToolsFromScanned(tools []ScannedTool) ([]ScannedTool, error) {
	...
	for _, scannedTool := range tools {
		// 检查这个扫描到的工具路径是否已经存在
		// 需要检查：1) 路径本身 2) 路径+可能的.app文件名
		scannedPath := filepath.ToSlash(scannedTool.Path)
		isExisting := existingToolPaths[scannedPath]

		// 如果路径不存在，还需要检查是否有Path+FileName的组合匹配
		if !isExisting {
			// 检查扫描路径的最后一部分是否是.app文件
			pathParts := strings.Split(scannedPath, "/")
			if len(pathParts) > 0 {
				lastPart := pathParts[len(pathParts)-1]
				if strings.HasSuffix(strings.ToLower(lastPart), ".app") {
					// 如果是.app，检查父目录+文件名是否已存在
					if len(pathParts) > 1 {
						parentPath := strings.Join(pathParts[:len(pathParts)-1], "/")
						combinedPath := filepath.ToSlash(filepath.Join(parentPath, lastPart))
						isExisting = existingToolPathFileNames[combinedPath]
					}
				} else {
					// 如果不是.app，检查是否有Path+FileName的组合
					// 尝试查找是否有工具使用这个路径作为Path，FileName为目录名
...
func (a *App) cleanInvalidToolPathsWithResult() (CleanupResult, error) {
	...
	for _, category := range categories.Category {
		...
		for _, tool := range category.Tool {
			...
			// 检查工具路径是否存在
			// 对于有FileName的工具（如.app文件），需要检查Path+FileName的组合路径
			...
			// 检查路径是否存在（.app是目录，需要特殊处理）
			pathExists := false
			if tool.FileName != "" && strings.HasSuffix(strings.ToLower(tool.FileName), ".app") {
				// .app文件是目录，检查目录是否存在
				if info, err := os.Stat(fullToolPath); err == nil {
					if info.IsDir() {
						pathExists = true
					} else {
						fmt.Printf("调试: .app路径不是目录 - 路径: %s\n", fullToolPath)
					}
				} else {
					// 添加调试信息
					fmt.Printf("调试: 检查.app路径失败 - Path: %s, FileName: %s, 完整路径: %s, 错误: %v\n",
				...
				if _, err := os.Stat(fullToolPath); err == nil {
					...
					// 添加调试信息
					fmt.Printf("调试: 检查路径失败 - Path: %s, FileName: %s, 完整路径: %s, 错误: %v\n",
						tool.Path, tool.FileName, fullToolPath, err)
				}
			}
			...
			if !pathExists {
				// 路径不存在，标记为无效
				fmt.Printf("发现无效工具路径: %s (工具: %s, FileName: %s, 完整路径: %s)\n", tool.Path, tool.Name, tool.FileName, fullToolPath)
	...
	for _, category := range categories.Category {
		...
		for _, tool := range category.Tool {
			...
			if !pathExists {
				...
				if filepath.IsAbs(tool.Path) {
					fmt.Printf("发现无效工具路径（绝对路径）: %s (工具: %s, FileName: %s, 完整路径: %s)\n", tool.Path, tool.Name, tool.FileName, fullToolPath)
					result.InvalidToolsCount++
					result.InvalidToolNames = append(result.InvalidToolNames, tool.Name)
				...
				// 没有找到迁移目标，按原逻辑处理
				fmt.Printf("发现无效工具路径: %s (工具: %s, FileName: %s)\n", tool.Path, tool.Name, tool.FileName)
				result.InvalidToolsCount++
				result.InvalidToolNames = append(result.InvalidToolNames, tool.Name)
