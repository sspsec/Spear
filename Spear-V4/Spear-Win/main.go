...
func main() {
	...
	addButton := widget.NewButton("添加工具", func() {
		...
		valueEntry.SetPlaceHolder("Java8/Java11/Python/Open/openterm")
...
func ExecuteCommand(path, optional, value, filename string) error {
	...
	switch value {
	case "Java8", "Java11":
		javaPath := filepath.Join(currentDir, config.Path.Java8)
		if value == "Java11" {
			javaPath = filepath.Join(currentDir, config.Path.Java11)
		}
		execCommand = fmt.Sprintf("cd %s && %s %s -jar %s", path, javaPath, optional, filename)
		cmd = exec.Command("cmd", "/C", execCommand)
	case "Open":
		execCommand = fmt.Sprintf("cd %s && %s %s", path, filename, optional)
		cmd = exec.Command("cmd", "/C", execCommand)
	case "Python3":
...
echo python %s %s
pause
`, currentDir+pythonPath, path, filename, filename, optional)
