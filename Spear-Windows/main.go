...
func (t customTheme) Font(style fyne.TextStyle) fyne.Resource {
	if style.Bold {
		return resourceMesloLGSNFRegularTtf // 这里使用正确的资源名称
	}
	if style.Italic {
		return resourceMesloLGSNFRegularTtf
	}
	if style.Monospace {
		return resourceMesloLGSNFRegularTtf
	}
...
func main() {
	...
	addButton := widget.NewButton("添加工具", func() {
		...
		valueEntry.SetPlaceHolder("Java8/Java11/Python3/Open/openterm")
...
func updateYAMLFile() {
	...
	data, err := yaml.Marshal(&struct {
		...
		Path:       config.Path, // 更新为新的 path 结构
	})
	if err != nil {
...
func ExecuteCommand(path, optional, value, filename string) error {
	...
	switch value {
	case "Java8", "Java11":
		javaPath := config.Path.Java8
		if value == "Java11" {
			javaPath = config.Path.Java11
		}
		execCommand = fmt.Sprintf("cd %s && %s %s -jar %s", fullPath, currentDir+javaPath, optional, filename)
		cmd = exec.Command("cmd", "/C", execCommand)
	case "Open":
		execCommand = fmt.Sprintf("cd %s && %s %s", fullPath, filename, optional)
		cmd = exec.Command("cmd", "/C", execCommand)
	case "Python3":
		pythonPath := config.Path.Python3
...
echo python %s %s
pause
`, currentDir+pythonPath, fullPath, filename, filename, optional)

		execCommand = filepath.Join(os.TempDir(), "run_python.bat")
		ioutil.WriteFile(execCommand, []byte(batchFileContent), 0644)

		// 通过 "start" 命令打开新的 cmd 窗口并执行命令
		cmd = exec.Command("cmd", "/C", "start", execCommand)
	...
	if err := cmd.Start(); err != nil {
		WriteErrorToFile("Command execution failed", "command.log", err)
		return err
	}
	return nil
}
