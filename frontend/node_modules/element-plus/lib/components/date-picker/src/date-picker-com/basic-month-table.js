  ...
  setup(__props, { expose, emit }) {
    ...
    const handleMonthTableClick = (event) => {
      ...
      if (props.selectionMode === "months") {
        ...
        const newValue = style.hasClass(target, "current") ? arrays.castArray(props.parsedValue).filter((d) => (d == null ? void 0 : d.month()) !== newMonth.month()) : arrays.castArray(props.parsedValue).concat([dayjs__default["default"](newMonth)]);
        emit("pick", newValue);
